...
- `npm run editor` 는 마크다운을 고치는 즉시 오른쪽 이력서가 바뀌고, 입력을 멈추면 0.8초 뒤 **자동 저장**됩니다
  (파일 기록 + `index.html` 재생성). **저장 · 분량 검사**(⌘S)를 누르면 거기에 더해 실제로 인쇄해 A4 쪽수를 잽니다
  — 크로미움을 띄우므로 자동 저장 때는 돌리지 않습니다. 편집기 밖에서 `projects.md` 가 바뀌면 덮어쓰지 않고 멈춥니다.
  **수정 프롬프트 복사** 는 지금 내용을 Claude Code 에 그대로 붙여 넣을 수 있는 지시문으로 만들어 줍니다.
