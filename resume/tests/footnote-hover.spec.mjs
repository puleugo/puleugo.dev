...
// 회귀 방지: sup.footnote a::after(닫는 대괄호)와 [data-tip]:hover::after(툴팁)가 같은 의사요소를
// 다투면, 각주에 호버하는 순간 "]" 가 사라지거나 툴팁으로 바뀌어 본문 표기가 깨졌다.
test("각주에 호버해도 대괄호 표기가 유지된다", async ({ page }) => {
	const fn = page.locator("sup.footnote").first();
	...
	await page.waitForSelector("#link-card.on");
...
test("각주 호버가 본문 레이아웃을 바꾸지 않는다", async ({ page }) => {
	...
	for (const i of [0, 2, 5]) {
		const link = page.locator("sup.footnote a").nth(i);
...
test("블로그 각주는 호버 시 미리보기 카드를 띄운다", async ({ page }) => {
	await page.locator('sup.footnote a[data-card]').first().hover();
